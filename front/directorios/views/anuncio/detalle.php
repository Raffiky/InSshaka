      ...
      <div id="contenu3">
        <div class="directorio-detalle">
          ...
          <?php if (!empty($datos->logo)) : ?>
            <div style="float:left;"><img width="120" src="<?php echo uploads_url($datos->logo) ?>" alt="" /></div>
          <?php endif; ?>
...
<script type="text/javascript">
 $('#map_canvas').gMap({
    markers: [
        {
            address: '<?= json_encode($datos->direccion) ?>',
            html: '<span style="color: #E82E7C; font-size:2.1em;"> <?= json_encode($datos->empresa)?> </span>'
        }
    ],
    address: '<?= json_encode($datos->direccion) ?>',
    zoom:12
